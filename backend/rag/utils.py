...
def _flatten_milvus_results(results: list) -> list:
    """将 Milvus 返回的嵌套结构展平为顶层字段。

    Milvus 返回: {'id': ..., 'distance': ..., 'entity': {'text': ..., 'filename': ...}}
    期望格式:    {'id': ..., 'distance': ..., 'text': ..., 'filename': ...}
    """
    flattened = []
    for item in results:
        flat = dict(item)
        entity = flat.pop("entity", {})
...
def _get_rerank_native_endpoint() -> str:
    return "https://dashscope.aliyuncs.com/api/v1/services/rerank/text-rerank/text-rerank"


def _use_native_rerank_api() -> bool:
...
#将细粒度分块（L3）向上合并到父分块（L2→L1），解决「碎片化检索结果」问题
...
#调用重排服务（基于配置的RERANK_MODEL），对初始检索结果按「与查询的相关性」重新排序；
def _rerank_documents(query: str, docs: List[dict], top_k: int) -> Tuple[List[dict], Dict[str, Any]]:
    ...
    use_native = _use_native_rerank_api()
    if use_native:
        payload = {
            "model": RERANK_MODEL,
            "input": {
            ...
            "parameters": {"top_n": min(top_k, len(docs_with_rank))},
        }
        endpoint = _get_rerank_native_endpoint()
    else:
        payload = {
            "model": RERANK_MODEL,
            "query": query,
    ...
    try:
        ...
        body = response.json()
        items = body.get("output", body).get("results", []) if use_native else body.get("results", [])
        reranked = []
        for item in items:
            idx = item.get("index")
...
def retrieve_documents(query: str, top_k: int = 5, intent_level: str = None, tenant_id: int = None) -> Dict[str, Any]:
    ...
    try:
        ...
        retrieved = _flatten_milvus_results(retrieved)
